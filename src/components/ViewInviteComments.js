...
function ViewInviteComments({ addingComment }) {
    ...
    useEffect(() => {
        if (page !== '') {
            (async () => {
                setLoading(true)
                try {
                    const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/comments/getComments/${fileId}`,
                        {
                            params: {
    ...
    }, [page, addingComment, fileId, inviteId])
...
export default ViewInviteComments